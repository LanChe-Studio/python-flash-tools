...
class ADB:
    ...
    def _connect(self):
        ...
        if not self._ensure_adb_server():
            raise Exception("ADB服务器不可用，请确保已安装ADB工具")
            ...
            if self.adb_path:
                try:
                    ...
                    if result.stdout.strip():
                        return [line.split('\t') for line in result.stdout.splitlines() if line and not line.startswith('List of')]
                except:
                    pass
            raise e
        finally:
...
class FlashTool(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Flash Tools V0.6")
        self.setGeometry(100, 100, 800, 600)
        
        # 初始化变量
        self.current_mode = None  # 'fastboot', 'bootrom', 'adb', None
        self.flashing = False
        self.firmware_path = ""
        self.adb = ADB()
        self.debug_mode = True  # 设为True可显示更多调试信息
