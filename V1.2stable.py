...
class FlashTool(QMainWindow):
    ...
    bootrom_log_signal = pyqtSignal(str)  # 新增：Bootrom日志信号
    
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Flash Tools V1.2")
        ...
        self.bootrom_log_signal.connect(self._update_bootrom_log)  # 连接Bootrom日志信号
    ...
    def _init_bootrom_tab(self, tab):
        ...
        self.start_detect_btn = QPushButton("开始识别Bootrom")
        self.stop_detect_btn = QPushButton("停止识别")
        self.stop_detect_btn.setEnabled(False)
        
        self.start_detect_btn.clicked.connect(self._start_detect_bootrom)
        self.stop_detect_btn.clicked.connect(self._stop_detect_bootrom)
        ...
        self.bootrom_status_label = QLabel("等待设备连接...")
        connection_layout.addWidget(self.bootrom_status_label)
        ...
        self.realtime_log = QPlainTextEdit()
        self.realtime_log.setReadOnly(True)
        self.realtime_log.setStyleSheet("font-family: monospace; font-size: 9pt;")
        realtime_layout.addWidget(self.realtime_log)
        realtime_group.setLayout(realtime_layout)
        
        # 组装布局
        layout.addWidget(info_label)
        layout.addLayout(detect_layout)  # 添加检测按钮
        layout.addWidget(connection_group)  # 设备连接状态区域
        layout.addWidget(partition_group)
        layout.addWidget(bootrom_group)
        layout.addWidget(realtime_group)  # 添加实时日志区域
        layout.addStretch()
                ...
                if self.mtk_detecting:  # 只有开始识别时才检测
                    try:
                        ...
                        if mtk_devices:
                            ...
                            self._update_bootrom_status(f"已连接MTK设备: {mtk_devices[0][0]}")  # 更新Bootrom状态
                        ...
                        self._update_bootrom_status(f"检测错误: {str(e)}")  # 更新Bootrom状态
                
                # 没有检测到设备
                self.mode_signal.emit(None, None)
                self.device_details.setText("设备详细信息将在此显示")
                self._update_bootrom_status("等待设备连接")  # 更新Bootrom状态
                time.sleep(3)
        
        self.check_thread = threading.Thread(target=check_loop, daemon=True)
        self.check_thread.start()
    
    def _update_bootrom_status(self, message):
        """更新Bootrom状态显示"""
        self.bootrom_status_label.setText(message)
    
    def _update_bootrom_log(self, message):
        """更新Bootrom日志显示"""
        self.realtime_log.appendPlainText(message)
    ...
    def _handle_mtk_device(self, device_id):
        ...
        self._update_button_states()
        self._update_bootrom_status(f"已连接MTK设备: {device_id}")  # 更新Bootrom状态
    ...
    def _update_button_states(self):
        ...
        self.dump_partition_btn.setEnabled(self.current_mode == "mtk")
        self.flash_partition_btn.setEnabled(self.current_mode == "mtk")
        self.get_partition_table_btn.setEnabled(self.current_mode == "mtk")
    ...
    def _show_about(self):
        """显示关于信息"""
        QMessageBox.information(self, "关于", "Python Flash Tools V1.2\nPowered by LanChe-Studio")
    ...
    def _start_detect_bootrom(self):
        """开始检测Bootrom设备"""
        self.log_signal.emit("开始持续检测Bootrom设备...")
        self.mtk_detecting = True
        self.start_detect_btn.setEnabled(False)
        self.stop_detect_btn.setEnabled(True)
        self._update_bootrom_status("正在检测Bootrom设备...")
    
    def _stop_detect_bootrom(self):
        """停止检测Bootrom设备"""
        self.log_signal.emit("已停止检测Bootrom设备")
        self.mtk_detecting = False
        self.start_detect_btn.setEnabled(True)
        self.stop_detect_btn.setEnabled(False)
        self._update_bootrom_status("检测已停止")
    ...
    def _start_bootrom_flashing(self):
        ...
        # 检查设备连接状态
        if self.current_mode != "mtk":
            QMessageBox.warning(self, "警告", "未检测到MTK设备，请连接设备并确保处于Bootrom模式")
            return
    ...
    def _execute_bootrom_flash(self):
        ...
        try:
            ...
            self.bootrom_log_signal.emit("开始Bootrom刷机...")
            ...
            try:
                ...
                if self.adb.mtk_path:
                    self.bootrom_log_signal.emit("正在使用MTKClient刷机...")
                    
                    # 构建命令
                    cmd = [sys.executable, self.adb.mtk_path, "flash", temp_dir]
                    self.bootrom_log_signal.emit(f"执行命令: {' '.join(cmd)}")
                    ...
                    while True:
                        ...
                        if output == '' and process.poll() is not None:
                            break
                        if output:
                            self.bootrom_log_signal.emit(output.strip())
                    
                    # 检查结果
                    if process.returncode == 0:
                        self.log_signal.emit("Bootrom刷机完成!")
                        self.bootrom_log_signal.emit("刷机成功完成!")
                    else:
                        self.log_signal.emit(f"Bootrom刷机失败，返回码: {process.returncode}")
                        self.bootrom_log_signal.emit(f"刷机失败，返回码: {process.returncode}")
                else:
            ...
            self.bootrom_log_signal.emit(f"刷机失败: {str(e)}")
        finally:
            self.operation_in_progress = False
    ...
    def _execute_dump_bootrom_partition(self):
        ...
        try:
            ...
            self.bootrom_log_signal.emit(f"开始提取 {partition} 分区...")
            ...
            if save_path:
                # 使用MTKClient提取分区
                if self.adb.mtk_path:
                    self.bootrom_log_signal.emit(f"正在使用MTKClient提取 {partition} 分区...")
                    
                    # 构建命令
                    cmd = [sys.executable, self.adb.mtk_path, "rf", partition, save_path]
                    self.bootrom_log_signal.emit(f"执行命令: {' '.join(cmd)}")
                    ...
                    if process.returncode == 0:
                        ...
                        self.bootrom_log_signal.emit("分区提取成功!")
                    else:
                        self.log_signal.emit(f"提取分区失败，返回码: {process.returncode}")
                        self.bootrom_log_signal.emit(f"提取失败，返回码: {process.returncode}")
                else:
                    self.log_signal.emit("未找到MTKClient工具")
            else:
            ...
            self.bootrom_log_signal.emit(f"提取失败: {str(e)}")
        finally:
            self.operation_in_progress = False
    ...
    def _execute_flash_bootrom_partition(self):
        ...
        try:
            ...
            self.bootrom_log_signal.emit(f"开始刷入 {partition} 分区...")
            
            # 使用MTKClient刷入分区
            if self.adb.mtk_path:
                self.bootrom_log_signal.emit(f"正在使用MTKClient刷入 {partition} 分区...")
                
                # 构建命令
                cmd = [sys.executable, self.adb.mtk_path, "wf", partition, self.partition_img_path]
                self.bootrom_log_signal.emit(f"执行命令: {' '.join(cmd)}")
                ...
                if process.returncode == 0:
                    ...
                    self.bootrom_log_signal.emit("分区刷入成功!")
                else:
                    self.log_signal.emit(f"刷入分区失败，返回码: {process.returncode}")
                    self.bootrom_log_signal.emit(f"刷入失败，返回码: {process.returncode}")
            else:
                self.log_signal.emit("未找到MTKClient工具")
        except Exception as e:
            self.log_signal.emit(f"刷入分区失败: {str(e)}")
            self.bootrom_log_signal.emit(f"刷入失败: {str(e)}")
        finally:
            self.operation_in_progress = False
    ...
    def _execute_get_bootrom_partition_table(self):
        ...
        try:
            ...
            self.bootrom_log_signal.emit("正在获取分区表信息...")
            
            # 使用MTKClient获取分区表
            if self.adb.mtk_path:
                self.bootrom_log_signal.emit("正在使用MTKClient获取分区表...")
                
                # 构建命令
                cmd = [sys.executable, self.adb.mtk_path, "printgpt"]
                self.bootrom_log_signal.emit(f"执行命令: {' '.join(cmd)}")
                ...
                if process.returncode == 0:
                    ...
                    self.partition_output.setPlainText("\n".join(output_lines))
                else:
                    self.log_signal.emit(f"获取分区表失败，返回码: {process.returncode}")
            else:
                self.log_signal.emit("未找到MTKClient工具")
        except Exception as e:
